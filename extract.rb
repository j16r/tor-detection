...
if ARGV.size != 1 || ARGV[0].match(/(-h|--help|\/?)/i) || !File.exists?(ARGV[0])
  puts <<-eol
Usage: #{$0} <filename>
Options:
  -h --help   Print this usage message

Extracts all percieved in text references from a text file and prints them to
stdout, sorted alphabetically
  eol
  exit
end

ReferenceExtractor.extract(ARGV[0])
