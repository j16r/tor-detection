...
network participants. In the academic literature, traditionally these networks
have been attacked with techniques that aim to uncover the identify of
participants. But the success of an anonymous network relies not only on it's
technical capabilities, but on adoption by a large enough user base to provide
adequate cover traffic. If anonymous network participants can be identified, the
users can be harassed, discouraging participation. Tor is an example of widely
used anonymous network which uses a form of Onion Routing to provide low latency
