...
of statistics to be used as attributes for classification. The attributes
produced by NetAI are shown in table \ref{table:netmate-attributes}.
...
\begin{table}[H]
  \begin{tabular}{lll}
    ...
    total\_fpackets & Total forward packets \\
    total\_fvolume & Total forward volume \\
    total\_bpackets & Total backward packets \\
    total\_bvolume & Total backward volume \\
    min\_fpktl & Minimum forward packet length \\
    mean\_fpktl & Mean forward packet length \\
    max\_fpktl & Maximum forward packet length \\
    std\_fpktl & Std. Deviation of forward packet length \\
    min\_bpktl & Minmum backward packet length \\
    mean\_bpktl & Mean backward packet length \\
    max\_bpktl & Maximum backward packet length \\
    std\_bpktl & Std. Deviation of backward packet length \\
    min\_fiat & Minimum forward inter arrival time \\
    mean\_fiat & Mean forward inter arrival time \\
    max\_fiat & Maximum forward inter arrival time \\
    std\_fiat & Std. Deviation of forward inter arrival time \\
    min\_biat & Minimum backward inter arrival time \\
    mean\_biat & Mean backward inter arrival time \\
    max\_biat & Maximum backward inter arrival time \\
    std\_biat & Std. Devation of backward inter arrival time \\
    duration & Session duration \\
    min\_active & Minimum active \\
    mean\_active & Mean active \\
    max\_active & Maximum active \\
    std\_active & Std. Deviation active \\
    min\_idle & Minimum idle \\
    mean\_idle & Mean idle \\
    max\_idle & Maximum idle \\
    std\_idle & Std. Deviation of idle \\
    sflow\_fpackets & Sub flow forward packets \\
    sflow\_fbytes & Sub flow forward bytes \\
    sflow\_bpackets & Sub flow backward packets \\
    sflow\_bbytes & Sub flow backward bytes \\
    fpsh\_cnt & Forward push counter \\
    bpsh\_cnt & Backward push counter \\
    furg\_cnt & Forward urgent counter \\
    burg\_cnt & Backward urgent counter \\
    total\_fhlen & Total forward header length \\
    total\_bhlen & Total backward header length \\
    \bottomrule
